...
var circleProgress = function (options) {
    ...
    var step = function () {
        if (options.current < options.progress && options.duration > 0) {
            _this.drawCircleProgress(options);
...
circleProgress.prototype.drawCircleProgress = function (options) {
    ...
    ctx.fillText('' + '17/40', x, y);

    ctx.restore();
};

circleProgress.prototype.forceRedraw = function (progress) {
    this.options.progress = progress;
    this.options.current = progress;
    this.drawCircleProgress(this.options);
